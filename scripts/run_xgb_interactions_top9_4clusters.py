#!/usr/bin/env python3
"""
Clustering en 4 clusters en utilisant uniquement les 9 premières interactions (joint_*) les plus fortes.

On repart de :
  figures/california_xgb_joint/joint_shap.csv

Étapes :
  - sélectionner les 9 colonnes joint_* correspondant aux interactions les plus fortes (RMS),
  - standardiser ces 9 dimensions,
  - UMAP 2D,
  - KMeans à 4 clusters sur l'espace UMAP,
  - générer UMAP, cartes, heatmap.

Sorties :
  figures/california_xgb_joint_4clusters_inter9/
"""
...
def _plot_heatmap(df: pd.DataFrame, label_col: str, comp_cols, path: str):
    ...
    ax.set_title(f"{len(clusters)} clusters × 9 interactions les plus fortes")
    plt.tight_layout()
    plt.savefig(path, dpi=180)
    plt.close()
...
def main():
    ...
    out_dir = os.path.join(base, "figures/california_xgb_joint_4clusters_inter9")
    os.makedirs(out_dir, exist_ok=True)

    df = pd.read_csv(in_csv)
    # On ne garde que les colonnes joint_* correspondant à des interactions (inter_: on ignore les main_)
    ...
    # UMAP
    reducer = create_reducer("umap", n_components=2, random_state=0, n_neighbors=40, min_dist=0.1)
    U = reducer.fit_transform(X9s)

    # KMeans à 4 clusters dans l'espace UMAP
    kmeans = KMeans(n_clusters=4, random_state=0, n_init=10)
    labels = kmeans.fit_predict(U)

    df_out = df.copy()
    df_out["umap_1_inter9"] = U[:, 0]
    df_out["umap_2_inter9"] = U[:, 1]
    df_out["cluster_inter9_4"] = labels

    # Sauvegarde CSV
    df_out.to_csv(os.path.join(out_dir, "joint_shap_4clusters_inter9.csv"), index=False)

    # Enregistrer la liste des 9 interactions utilisées
    with open(os.path.join(out_dir, "top9_interactions.txt"), "w") as f:
        f.write("Top 9 joint_* interactions by RMS:\n")
        for c in top9_cols:
            f.write(c + "\n")

    # Figures
    _plot_umap(df_out, "umap_1_inter9", "umap_2_inter9", "cluster_inter9_4",
               os.path.join(out_dir, "umap_cluster.png"),
               "UMAP (9 interactions les plus fortes) – 4 clusters (KMeans)")
    _plot_umap(df_out, "umap_1_inter9", "umap_2_inter9", "price",
               os.path.join(out_dir, "umap_price.png"),
               "UMAP (9 interactions les plus fortes) – couleur = prix")
    _plot_map(df_out, "cluster_inter9_4",
              os.path.join(out_dir, "map_cluster.png"),
              "Carte Californie – 4 clusters (9 interactions les plus fortes)")
    _plot_map(df_out, "price",
              os.path.join(out_dir, "map_price.png"),
              "Carte Californie – prix")
    _plot_heatmap(df_out, "cluster_inter9_4", top9_cols,
                  os.path.join(out_dir, "heatmap_cluster_vs_interactions.png"))

    print("Terminé. Résultats (4 clusters avec 9 interactions) dans", out_dir)
